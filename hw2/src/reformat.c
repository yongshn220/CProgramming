...
/* Determine maximum length of the shortest line: */
  ...
  for ( ;  w1 != head;  w1 = w1->prev) {
    ...
    if (w1->score < 0) {
      ...
      sprintf(tmperrm,impossibility,1);
      set_error(tmperrm);
      free(tmperrm);
      return 0;
    }
  ...
  /* Determine the minimum possible longest line: */
    ...
    if (newL > L) {
      tmperrm = calloc(100, sizeof(char));
      sprintf(tmperrm,impossibility,2);
      set_error(tmperrm);
      free(tmperrm);
      return 0;
    }
...
/* between newL and the lengths of the lines:         */
  ...
  if (head->next && head->next->score < 0) {
    tmperrm = calloc(100, sizeof(char));
    sprintf(tmperrm,impossibility,3);
    set_error(tmperrm);
    free(tmperrm);
    return 0;
  }
                ...
                int prefix, int suffix, int hang, int last, int min)
{
  ...
  char *q1, *q2, **outlines = NULL; // (yongjung: initialized outlines as NULL)
